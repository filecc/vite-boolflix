        ...
        <Transition>
            <div v-if="!GENERAL.searchingMode" class="p-4">
                ...
                <div v-for="item in arrayOfMoviesPerGenres">
                    ...
                    <div :id="item.name.split(' ')[0]" @wheel.stop="e => scroll(e, item.name.split(' ')[0], true)"
                        ref="boxes" class="containerPopular">
                        <router-link @wheel.stop="e => scroll(e, item.name.split(' ')[0], true)"
                            v-for="movie in item.movies" :to="'/movie/' + movie.id + '-' + movie.title.split('.')[0]">
                            <SingleMovieCard :item="movie" :image="movie.poster_path" />
                        </router-link>
                    </div>
                </div>
